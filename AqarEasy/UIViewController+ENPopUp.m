...
- (void)dismissPopUpViewControllerWithcompletion:(void (^)(void))completionBlock
{
    ...
    if (!blurView) {
        //UIViewController *presenter = [self.presentingViewController viewControllers][0]; // 0 is assuming that ParentViewController is in the first tab. Change if necessary

        UIView *sourceView =self.presentingViewController.view;
        blurView = (JWBlurView *)[sourceView viewWithTag:kENPopUpBluredViewTag];
...
- (UIView*)topView {
    UIViewController *recentView = self;//[self en_popupViewController];
